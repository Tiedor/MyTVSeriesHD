...
public class MyBucketEpisodeAdapter extends BucketListAdapter<Episode> {
	...
	private	static	final 	String 					LOG_TAG 	= MyConstants.LOG_TAG + " - " + MyBucketEpisodeAdapter.class.getSimpleName();

	public MyBucketEpisodeAdapter(Activity ctx, ArrayList<Episode> elements,
			Integer bucketSize) {
	...
	protected View getBucketElement(int position, Episode currentElement,
		...
		viewHolder.text.setText(currentElement.getEpisodeName());
		...
		if(currentElement.getAirTime() != null && today.getTime().after(currentElement.getAirTime())){
			...
			if(!currentElement.isDownloading()){
			...
			}else if(!currentElement.isSub()){
				viewHolder.light.setImageResource(R.drawable.yellow_light);
				if(currentElement.getItasaSubUrl() != null && !currentElement.getItasaSubUrl().equals("")){
				...
				viewHolder.light.setImageResource(R.drawable.green_light);
	...
	public static class PosterContextMenuInfo implements ContextMenu.ContextMenuInfo {  
        public PosterContextMenuInfo(View targetView) {  
